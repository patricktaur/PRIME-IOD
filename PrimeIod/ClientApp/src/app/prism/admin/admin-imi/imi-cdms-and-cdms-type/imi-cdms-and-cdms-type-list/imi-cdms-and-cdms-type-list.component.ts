...
import { imicdmsandcdmstypefilters } from '@app/prism/admin/admin-imi/imi-cdms-and-cdms-type/imi-cdms-and-cdms-type-filter/imicdmsandcdmstypefilters';

@Component({
  selector: 'app-imi-cdms-and-cdms-type-list',
  templateUrl: './imi-cdms-and-cdms-type-list.component.html',
  styleUrls: ['./imi-cdms-and-cdms-type-list.component.css']
})
export class ImiCdmsAndCdmsTypeListComponent implements OnInit {
  title = 'abcd';
  isLoading = false;
  records: any;
  ...
  selectedFilters: imicdmsandcdmstypefilters = {
    pageNumber: 1,
    pageSize: 10
    // iconNumberOrName: '',
    // canLoginId: [],
  ...
  constructor(
    ...
    private imicdmsandcdmstypeService: imicdmsandcdmstypeService // public actRoute: ActivatedRoute, // public studyEditService: StudyEditAService
  ) {
  ...
  loadReport() {
    this.isLoading = true;
    this.imicdmsandcdmstypeService.GetList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  onFilterChange(filters: imicdmsandcdmstypefilters) {
