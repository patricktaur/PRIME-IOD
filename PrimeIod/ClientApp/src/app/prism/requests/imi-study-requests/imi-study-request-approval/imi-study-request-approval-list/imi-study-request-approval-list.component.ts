...
@Component({
  selector: 'app-imi-study-request-approval-list',
  templateUrl: './imi-study-request-approval-list.component.html',
  styleUrls: ['./imi-study-request-approval-list.component.css']
})
export class ImiStudyRequestApprovalListComponent implements OnInit {
  title = 'abcd';
  isLoading = false;
  records: any;
  ...
  constructor(
    ...
    private requestsService: RequestsIMIStudyService // public actRoute: ActivatedRoute, // public studyEditService: StudyEditAService
  ) {}
  ...
  loadReport() {
    this.isLoading = true;
    this.requestsService.getImiStudyRequestsList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
