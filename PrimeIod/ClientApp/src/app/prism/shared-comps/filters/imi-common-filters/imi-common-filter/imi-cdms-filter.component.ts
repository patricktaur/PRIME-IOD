...
@Component({
  selector: 'app-imi-cdms-filter',
  templateUrl: './imi-cdms-filter.component.html',
  styleUrls: ['./imi-cdms-filter.component.css']
})
export class ImiCdmsFilterComponent implements OnInit {
  ...
  loadFilters() {
    ...
    this.sharedCompsService.getImiCdmsFilters().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  get filterRecords() {
    ...
    if (this.dmpmsFilters && this.dmpmsFilters.length > 0) {
      ...
      filter5 = filter4.filter((n: any) => this.dmpmsFilters.some((v: any) => n.cdmsLead?.includes(v)));
    }
    ...
    if (this.dmpmManagerFilters && this.dmpmManagerFilters.length > 0) {
      ...
      filter6 = filter5.filter((n: any) => this.dmpmManagerFilters.some((v: any) => n.coveringCdmsLead?.includes(v)));
    }
