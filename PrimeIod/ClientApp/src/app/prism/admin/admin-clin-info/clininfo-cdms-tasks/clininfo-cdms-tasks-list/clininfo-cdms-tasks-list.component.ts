...
// import { clininfocdmstasksfilters } from '@app/prism/admin/cdms/clininfo-cdms-tasks/clininfo-cdms-tasks-filter/clininfocdmstasksfilters';

import { clininfocdmstasksService } from '@app/prism/admin/admin-clin-info/clininfo-cdms-tasks/clininfo-cdms-tasks.service';
import { clininfocdmstasksfilters } from '@app/prism/admin/admin-clin-info/clininfo-cdms-tasks/clininfo-cdms-tasks-filter/clininfocdmstasksfilters';


@Component({
  selector: 'app-clininfo-cdms-tasks-list',
  templateUrl: './clininfo-cdms-tasks-list.component.html',
  styleUrls: ['./clininfo-cdms-tasks-list.component.css']
})
export class ClininfoCdmsTasksListComponent {
  title = 'abcd';
  isLoading = false;
  records: any;
  ...
  selectedFilters: clininfocdmstasksfilters = {
    pageNumber: 1,
    pageSize: 10
    // iconNumberOrName: '',
    // canLoginId: [],
  ...
  constructor(
    ...
    private clininfocdmstasksService: clininfocdmstasksService // public actRoute: ActivatedRoute, // public studyEditService: StudyEditAService
  ) {
  ...
  loadReport() {
    this.isLoading = true;
    this.clininfocdmstasksService.GetList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  onFilterChange(filters: clininfocdmstasksfilters) {
