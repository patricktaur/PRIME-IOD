...
import { deactivateRequestFilters } from '@app/prism/requests/user-requests/deactivate/deactivate-request-filter/deactivate-request-filters';

@Component({
  selector: 'app-deactivate-request-list',
  templateUrl: './deactivate-request-list.component.html',
  styleUrls: ['./deactivate-request-list.component.css']
})
export class DeactivateRequestListComponent implements OnInit {
  title = 'abcd';
  isLoading = false;
  records: any;
  ...
  selectedFilters: deactivateRequestFilters = {
    pageNumber: 1,
    pageSize: 10,
    iconNumberOrName: '',
    status: [0],
  ...
  loadReport() {
    this.isLoading = true;
    this.requestsService.DeactivateRequestUserList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  onFilterChange(filters: deactivateRequestFilters) {
  ...
  add() {
    this.router.navigate(['deactivate-request-edit'], {
  ...
  edit(id: number) {
    this.router.navigate(['deactivate-request-edit'], { relativeTo: this.actRoute.parent, queryParams: { id: id } });
  }

  deleteRecord(recId: number) {
    this.deleteRecordSub = this.requestsService.DeactivateRequestUserDelete(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  columns: Array<any> = [
    ...
    // },
    {
      ...
      field: 'userOperatingDivisionPDescription'
    },
    {
      header: 'Office Region',
      field: 'userOfficeRegionPDescription'
    },
    {
      header: 'Office Country',
      field: 'userOfficeCountryPDescription'
    },

    {
      header: 'Time Zone',
      field: 'userTimeZoneId',
      hide: true
    },
    {
