...
@Component({
  selector: 'app-crm-codelist-list',
  templateUrl: './crm-codelist-list.component.html',
  styleUrls: ['./crm-codelist-list.component.css']
})
export class CrmCodelistListComponent {
  ...
  loadReport() {
    ...
    if (this.selectedFilters.parId != null) {
      this.crmCodelistService.getdataforParid(this.selectedFilters.parId)
      .subscribe(
        (res: any) => {
          if (res.status === 400) {
            this.addbtn = false;
  ...
  onFilterChange(filters: any) {
    this.crmCodelistFilterService.setFilters(filters);
    this.selectedFilters = filters;
    //alert(this.selectedFilters.parId);
    this.loadReport();
  }
  ...
  onResetFilters() {
    this.crmCodelistFilterService.resetFilters();
  }
