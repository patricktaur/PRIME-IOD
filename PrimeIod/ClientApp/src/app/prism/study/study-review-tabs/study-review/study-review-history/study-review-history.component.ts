...
@Component({
  selector: 'app-study-review-history',
  templateUrl: './study-review-history.component.html',
  styleUrls: ['./study-review-history.component.css']
})
export class StudyReviewHistoryComponent implements OnInit, OnDestroy {
  studyId: number = 0;
  ...
  loadStudyReviewHistory(studyId: number) {
    this.loading = true;
    this.loadSub = this.studyReviewService.getStudyReviewHistory(studyId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
