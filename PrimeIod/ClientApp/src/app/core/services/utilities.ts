...
export class Utilities {
      ...
      if (this.checkNoNetwork(data)) {
        ...
        if (responseObject && (typeof responseObject === 'object' || responseObject instanceof Object)) {
          for (let key in responseObject) {
            if (key) responses.push(`${key}${this.captionAndMessageSeparator} ${responseObject[key]}`);
            else if (responseObject[key]) responses.push(responseObject[key].toString());
          }
        }
      }
  ...
  public static getQueryParamsFromString(paramString: string) {
    ...
    for (let param of paramString.split('&')) {
      let keyValue: any = Utilities.splitInTwo(param, '=');
  ...
  public static splitInTwo(text: string, separator: string): { firstPart: string; secondPart: string | null } {
    ...
    if (separatorIndex == -1) return { firstPart: text, secondPart: null };
  ...
  public static baseUrl() {
    ...
    if (window.location.origin) base = window.location.origin;
    else
      base =
  ...
  public static printFriendlyDate(date: Date, separator = '-') {
    ...
    if (test.toDateString() == today.toDateString()) return `Today ${separator} ${Utilities.printTimeOnly(date)}`;
    if (test.toDateString() == yesterday.toDateString())
      return `Yesterday ${separator} ${Utilities.printTimeOnly(date)}`;
    else return Utilities.printDate(date, separator);
  }
  ...
  public static parseDate(date: any): Date | any {
    if (date) {
      ...
      if (typeof date === 'string' || date instanceof String) {
        if (date.search(/[a-su-z+]/i) == -1) date = date + 'Z';
  ...
  public static printDuration(start: Date, end: Date) {
    ...
    if (days) printedDays = `${days} days`;

    if (hours) printedDays += printedDays ? `, ${hours} hours` : `${hours} hours`;

    if (minutes) printedDays += printedDays ? `, ${minutes} minutes` : `${minutes} minutes`;

    if (seconds) printedDays += printedDays ? ` and ${seconds} seconds` : `${seconds} seconds`;

    if (!printedDays) printedDays = '0';
  ...
  public static searchArray(searchTerm: string, caseSensitive: boolean, ...values: any[]) {
    ...
    if (!caseSensitive) searchTerm = searchTerm.toLowerCase();
    ...
    for (let value of values) {
      if (value != null) {
        ...
        if (strValue.indexOf(searchTerm) !== -1) return true;
      }
    }
  ...
  public static debounce(func: (...args: any) => any, wait: number, immediate?: boolean) {
    ...
    return function() {
      ...
      /*****  Surekha ... need to test *****/
      var [args_] = arguments;

      var later = function() {
        timeout = null;
        if (!immediate) func.apply(context, args_);
      };
      ...
      if (callNow) func.apply(context, args_);
    };
  }
