...
export class LocalStoreManager {
  ...
  public initialiseStorageSyncListener() {
    if (LocalStoreManager.syncListenerInitialised == true) return;
  ...
  private sessionStorageTransferHandler = (event: StorageEvent) => {
    ...
    if (event.key == 'getSessionStorage') {
      ...
      if (!this.syncKeys.length) this.loadSyncKeys();

      let data = JSON.parse(event.newValue);
      //console.info("Set => Key: Transfer setSessionStorage" + ",  data: " + JSON.stringify(data));

      for (let key in data) {
        if (this.syncKeysContains(key)) this.sessionStorageSetItem(key, JSON.parse(data[key]));
      }
  ...
  private loadSyncKeys() {
    if (this.syncKeys.length) return;
  ...
  private localStorageGetItem(key: string) {
    ...
    if(localStorageItem != null) {
      return Utilities.JSonTryParse(localStorageItem);
    }
  }
  ...
  private sessionStorageGetItem(key: string) {
    ...
    if(sessionStorageItem) {
      return Utilities.JSonTryParse(sessionStorageItem);
    }
  }
