...
@Component({
  selector: 'app-imi-resources-s-filter',
  templateUrl: './imi-resources-s-filter.component.html',
  styleUrls: ['./imi-resources-s-filter.component.css']
})
export class ImiResourcesSFilterComponent implements OnInit {
    ...
    this.sharedCompsService.getDMFilters().subscribe(
      (res: any) => {
        if (res.status === 400) {
          ...
          //
          let itemInList = this.hasResourceInList(this.filters.imiResources, this.currentUserId);
          if (!itemInList) {
