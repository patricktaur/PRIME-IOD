...
// import { clininfocdmstaskgroupsfilters } from '@app/prism/admin/cdms/clininfo-cdms-task-groups/clininfo-cdms-task-groups-filter/clininfocdmstaskgroupsfilters';
...
import {clininfocdmstaskgroupsfilters} from '@app/prism/admin/admin-clin-info/clininfo-cdms-task-groups/clininfo-cdms-task-groups-filter/clininfocdmstaskgroupsfilters'

@Component({
  selector: 'app-clininfo-cdms-task-groups-list',
  templateUrl: './clininfo-cdms-task-groups-list.component.html',
  styleUrls: ['./clininfo-cdms-task-groups-list.component.css']
})
export class ClininfoCdmsTaskGroupsListComponent {
  title = 'abcd';
  isLoading = false;
  records: any;
  ...
  selectedFilters: clininfocdmstaskgroupsfilters = {
    pageNumber: 1,
    pageSize: 10
    // iconNumberOrName: '',
    // canLoginId: [],
  ...
  constructor(
    ...
    private clininfocdmstaskgroupsservice: clininfocdmstaskgroupsservice // public actRoute: ActivatedRoute, // public studyEditService: StudyEditAService
  ) {
  ...
  loadReport() {
    this.isLoading = true;
    this.clininfocdmstaskgroupsservice.GetList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  onFilterChange(filters: clininfocdmstaskgroupsfilters) {
