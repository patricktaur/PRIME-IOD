...
@Component({
  selector: 'app-third-party-cdms-task-group-s-filter',
  templateUrl: './third-party-cdms-task-group-s-filter.component.html',
  styleUrls: ['./third-party-cdms-task-group-s-filter.component.css']
})
export class ThirdPartyCdmsTaskGroupSFilterComponent implements OnInit {
  ...
  loadFilters() {
    this.sharedCompsService.getThirdPartyCdmsTaskGroupFilters().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
