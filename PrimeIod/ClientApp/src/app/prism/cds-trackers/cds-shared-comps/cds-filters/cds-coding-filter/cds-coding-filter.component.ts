...
@Component({
  selector: 'app-cds-coding-filter',
  templateUrl: './cds-coding-filter.component.html',
  styleUrls: ['./cds-coding-filter.component.css']
})
export class CdsCodingFilterComponent implements OnInit, OnDestroy {
  // @Input() requiredPermissions: any;
  // @Input() selectedFilters: any;
  @Input() selectedFilters: CdsCodingPaginationAndFilters = {
    pageNumber: 1,
    pageSize: 10,
    searchOn: 'task-id',
    sortOn: 'task-id',
  ...
  loadFilters() {
    this.loadSub = this.cdsTrackersService.getCdsCodingTrackerFilters().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
