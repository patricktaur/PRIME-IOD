...
export class UpdateRequestEditComponent implements OnInit {
  ...
  onChildItemChange(value: any) {
    if (value) {
      ...
      this.record.previousOrderNumber = value.orderNumber;
      this.record.previousDescription = value.description;
      this.form.get('orderNumber').setValue(value.orderNumber);
      this.form.get('description').setValue(value.description);
    } else {
      this.record.pid = 0;
      this.record.orderNumber = '';
      this.record.description = '';
      this.record.previousOrderNumber = 0;
      this.record.previousDescription = '';
  ...
  loadNewRecord() {
    this.loading = true;
    this.loadNewRecordSub = this.requestsService.UpdateRequestCodelistNew().subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  showChangeAlert() {
    return this.record?.recId > 0 && this.record?.isActioned == 0 ? true : false;
  }
  ...
  get submitForApprovalButtonEnabled() {
    return this.record?.description?.length > 0 && this.record?.pid > 0 ? true : false;
  }
