...
import { UserRequestFilters } from '@app/prism/requests/codelist-requests/approval/approval-request-filter/approval-request-filters';
...
export class ApprovalRequestListComponent implements OnInit {
  ...
  loadReport() {
    this.isLoading = true;
    this.requestsService.ApprovalRequestCodelistList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
