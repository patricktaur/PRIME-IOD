...
import { CodelistRequestFilters } from '@app/prism/requests/codelist-requests/deactivate/deactivate-request-filter/codelist-request-filters';
...
export class DeactivateRequestListComponent implements OnInit {
  ...
  loadReport() {
    ...
    this.requestsService.DeactivateRequestCodelistList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
