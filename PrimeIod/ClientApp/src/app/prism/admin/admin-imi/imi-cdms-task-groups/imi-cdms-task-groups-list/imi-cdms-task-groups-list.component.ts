...
import { imicdmstaskgroupsfilters } from '@app/prism/admin/admin-imi/imi-cdms-task-groups/imi-cdms-tasks-groups-filter/imicdmstaskgroupsfilters';

@Component({
  selector: 'app-imi-cdms-task-groups-list',
  templateUrl: './imi-cdms-task-groups-list.component.html',
  styleUrls: ['./imi-cdms-task-groups-list.component.css']
})
export class ImiCdmsTaskGroupsListComponent implements OnInit {
  title = 'abcd';
  isLoading = false;
  records: any;
  ...
  selectedFilters: imicdmstaskgroupsfilters = {
    pageNumber: 1,
    pageSize: 10
    // iconNumberOrName: '',
    // canLoginId: [],
  ...
  constructor(
    ...
    private imicdmstaskgroupsService: imicdmstaskgroupsService // public actRoute: ActivatedRoute, // public studyEditService: StudyEditAService
  ) {
  ...
  loadReport() {
    this.isLoading = true;
    this.imicdmstaskgroupsService.GetList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  onFilterChange(filters: imicdmstaskgroupsfilters) {
