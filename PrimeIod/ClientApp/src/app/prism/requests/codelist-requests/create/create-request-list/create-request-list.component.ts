...
import { UserRequestFilters } from '@app/prism/requests/codelist-requests/create/create-request-filter/user-request-filters';
import { CredentialsService } from '@app/core/authentication/credentials.service';

@Component({
  selector: 'app-create-request-list',
  templateUrl: './create-request-list.component.html',
  styleUrls: ['./create-request-list.component.css']
})
export class CreateRequestListComponent implements OnInit {
  title = 'abcd';
  isLoading = false;
  LoggedinUserId = '';
  records: any;
  ...
  loadReport() {
    ...
    this.requestsService.CreateRequestCodelistList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
