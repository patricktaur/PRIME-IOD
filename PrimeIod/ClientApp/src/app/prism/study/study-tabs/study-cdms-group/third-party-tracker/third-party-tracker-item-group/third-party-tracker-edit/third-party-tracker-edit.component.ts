...
@Component({
  selector: 'app-third-party-tracker-edit',
  templateUrl: './third-party-tracker-edit.component.html',
  styleUrls: ['./third-party-tracker-edit.component.css']
})
export class ThirdPartyTrackerEditComponent implements OnInit, OnDestroy {
  title = 'Cdms Third Tracker';
  controllerName = 'TblThirdPartyCdmstrackerMainRecords';
  ...
  constructor(
    ...
    private thirdPartyTrackerService: ThirdPartyTrackerService,
  ...
  CheckIfCPPCNumberExists() {
    ...
    this.checkCppcExistsSub = this.thirdPartyTrackerService
  ...
  loadTaskGroups(studyId: number) {
    this.loadNewRecordSub = this.thirdPartyTrackerService.getTaskGroups(studyId).subscribe(
      (res: any) => {
        this.taskGroups = res;
      },
      (err: any) => {
  ...
  loadNewRecord(studyId: any, taskGroupId: number) {
    this.loading = true;
    this.loadNewRecordSub = this.thirdPartyTrackerService.getNew(studyId, taskGroupId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
