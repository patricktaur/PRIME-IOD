...
import { cdmstaskgroupsfilters } from '@app/prism/admin/admin-cdms/cdms-task-groups/cdms-tasks-groups-filter/cdmstaskgroupsfilters';

@Component({
  selector: 'app-cdms-task-groups-list',
  templateUrl: './cdms-task-groups-list.component.html',
  styleUrls: ['./cdms-task-groups-list.component.css']
})
export class CdmsTaskGroupsListComponent implements OnInit {
  title = 'abcd';
  isLoading = false;
  records: any;
  ...
  selectedFilters: cdmstaskgroupsfilters = {
    pageNumber: 1,
    pageSize: 10
    // iconNumberOrName: '',
    // canLoginId: [],
  ...
  constructor(
    ...
    private cdmstaskgroupsService: cdmstaskgroupsService // public actRoute: ActivatedRoute, // public studyEditService: StudyEditAService
  ) {
  ...
  loadReport() {
    this.isLoading = true;
    this.cdmstaskgroupsService.GetList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  onFilterChange(filters: cdmstaskgroupsfilters) {
