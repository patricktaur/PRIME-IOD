...
@Component({
  selector: 'app-cdms-tasks-edit',
  templateUrl: './cdms-tasks-edit.component.html',
  styleUrls: ['./cdms-tasks-edit.component.css']
})
export class CdmsTasksEditComponent implements OnInit {
  loading: boolean = false;
  title = 'Edit CDMS Tasks';
  ...
  loadRecord(recId: number) {
    this.loading = true;
    this.loadRecordSub = this.cdmstasksService.GetById(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  loadNewRecord() {
    this.loading = true;
    this.loadRecordSub = this.cdmstasksService.NewCDMSTasksRequest().subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  addOrUpdate() {
    ...
    this.addUpdateSub = this.cdmstasksService.AddorUpdate(this.record).subscribe(
      res => {
        this.form.reset();
        this.location.back();
        this.loading = false;
  ...
  deleteRecord(recId: number) {
    this.loading = true;
    this.deleteRecordSub = this.cdmstasksService.deleteRec(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
