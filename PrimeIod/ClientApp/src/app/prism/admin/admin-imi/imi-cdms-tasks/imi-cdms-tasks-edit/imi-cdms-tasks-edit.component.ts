...
import { imicdmstasksService } from '@app/prism/admin/admin-imi/imi-cdms-tasks/imi-cdms-tasks.service';
...
@Component({
  selector: 'app-imi-cdms-tasks-edit',
  templateUrl: './imi-cdms-tasks-edit.component.html',
  styleUrls: ['./imi-cdms-tasks-edit.component.css']
})
export class ImiCdmsTasksEditComponent implements OnInit {
  loading: boolean = false;
  title = 'Edit IMI CDMS Tasks';
  ...
  constructor(
    ...
    private imicdmstasksService: imicdmstasksService,
  ...
  loadRecord(recId: number) {
    this.loading = true;
    this.loadRecordSub = this.imicdmstasksService.GetById(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  loadNewRecord() {
    this.loading = true;
    this.loadRecordSub = this.imicdmstasksService.NewCDMSTasksRequest().subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  addOrUpdate() {
    ...
    this.addUpdateSub = this.imicdmstasksService.AddorUpdate(this.record).subscribe(
      res => {
        this.form.reset();
        this.location.back();
        this.loading = false;
  ...
  deleteRecord(recId: number) {
    this.loading = true;
    this.deleteRecordSub = this.imicdmstasksService.deleteRec(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
