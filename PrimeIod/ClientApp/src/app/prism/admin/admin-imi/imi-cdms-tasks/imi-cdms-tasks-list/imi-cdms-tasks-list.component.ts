...
import { imicdmstasksfilters } from '@app/prism/admin/admin-imi/imi-cdms-tasks/imi-cdms-tasks-filter/imicdmstasksfilters';

@Component({
  selector: 'app-imi-cdms-tasks-list',
  templateUrl: './imi-cdms-tasks-list.component.html',
  styleUrls: ['./imi-cdms-tasks-list.component.css']
})
export class ImiCdmsTasksListComponent implements OnInit {
  title = 'abcd';
  isLoading = false;
  records: any;
  ...
  selectedFilters: imicdmstasksfilters = {
    pageNumber: 1,
    pageSize: 10
    // iconNumberOrName: '',
    // canLoginId: [],
  ...
  constructor(
    ...
    private imicdmstasksService: imicdmstasksService // public actRoute: ActivatedRoute, // public studyEditService: StudyEditAService
  ) {
  ...
  loadReport() {
    this.isLoading = true;
    this.imicdmstasksService.GetList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  onFilterChange(filters: imicdmstasksfilters) {
