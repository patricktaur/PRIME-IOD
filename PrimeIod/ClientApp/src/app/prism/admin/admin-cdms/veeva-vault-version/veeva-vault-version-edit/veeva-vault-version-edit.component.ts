...
@Component({
  selector: 'app-veeva-vault-version-edit',
  templateUrl: './veeva-vault-version-edit.component.html',
  styleUrls: ['./veeva-vault-version-edit.component.css']
})
export class VeevaVaultVersionEditComponent implements OnInit {
  loading: boolean = false;
  title = 'Edit Veeva Vault Version';
  ...
  constructor(
    ...
    private veevavaultversionService: veevavaultversionService,
  ...
  loadRecord(recId: number) {
    this.loading = true;
    this.loadRecordSub = this.veevavaultversionService.GetById(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  loadNewRecord() {
    this.loading = true;
    this.loadRecordSub = this.veevavaultversionService.NewCDMSTasksRequest().subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  addOrUpdate() {
    ...
    this.addUpdateSub = this.veevavaultversionService.AddorUpdate(this.record).subscribe(
      res => {
        this.form.reset();
        this.location.back();
        this.loading = false;
  ...
  deleteRecord(recId: number) {
    this.loading = true;
    this.deleteRecordSub = this.veevavaultversionService.deleteRec(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  fields: FormlyFieldConfig[] = [
    ...
    {
      key: 'iconOwnedVaultId',
      type: 'select',
      wrappers: ['horizontal-layout'],
      templateOptions: {
        label: 'ICON Owned Value',
        //required: true,

        options: this.tblParamService.getParams(this.YesNoPid),
        valueProp: 'recId',
