...
import { cdmsandcdmstypeService } from '@app/prism/admin/admin-cdms/cdms-and-cdms-type/cdms-and-cdms-type.service';
import { cdmsandcdmstypefilters } from '@app/prism/admin/admin-cdms/cdms-and-cdms-type/cdms-and-cdms-type-filter/cdmsandcdmstypefilters';

@Component({
  selector: 'app-cdms-and-cdms-type-list',
  templateUrl: './cdms-and-cdms-type-list.component.html',
  styleUrls: ['./cdms-and-cdms-type-list.component.css']
})
export class CdmsAndCdmsTypeListComponent implements OnInit {
  title = 'abcd';
  isLoading = false;
  records: any;
  ...
  selectedFilters: cdmsandcdmstypefilters = {
    pageNumber: 1,
    pageSize: 10
    // iconNumberOrName: '',
    // canLoginId: [],
  ...
  constructor(
    ...
    private cdmsandcdmstypeService: cdmsandcdmstypeService // public actRoute: ActivatedRoute, // public studyEditService: StudyEditAService
  ) {
  ...
  loadReport() {
    this.isLoading = true;
    this.cdmsandcdmstypeService.GetList().subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
  ...
  onFilterChange(filters: cdmsandcdmstypefilters) {
