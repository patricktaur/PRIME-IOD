...
@Component({
  selector: 'app-cdms-and-cdms-type-edit',
  templateUrl: './cdms-and-cdms-type-edit.component.html',
  styleUrls: ['./cdms-and-cdms-type-edit.component.css']
})
export class CdmsAndCdmsTypeEditComponent implements OnInit {
  loading: boolean = false;
  title = 'Edit CDMS and CDMS Type';
  ...
  constructor(
    ...
    private cdmsandcdmstypeService: cdmsandcdmstypeService,
  ...
  loadRecord(recId: number) {
    this.loading = true;
    this.loadRecordSub = this.cdmsandcdmstypeService.GetById(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  loadNewRecord() {
    this.loading = true;
    this.loadRecordSub = this.cdmsandcdmstypeService.NewCDMSTasksRequest().subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  addOrUpdate() {
    ...
    this.addUpdateSub = this.cdmsandcdmstypeService.AddorUpdate(this.record).subscribe(
      res => {
        this.form.reset();
        this.location.back();
        this.loading = false;
  ...
  deleteRecord(recId: number) {
    this.loading = true;
    this.deleteRecordSub = this.cdmsandcdmstypeService.deleteRec(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
