...
@Component({
  selector: 'app-clininfo-cdms-tasks-edit',
  templateUrl: './clininfo-cdms-tasks-edit.component.html',
  styleUrls: ['./clininfo-cdms-tasks-edit.component.css']
})
export class ClininfoCdmsTasksEditComponent {
  loading: boolean = false;
  title = 'Edit Clininfo CDMS Tasks';
  ...
  loadRecord(recId: number) {
    this.loading = true;
    this.loadRecordSub = this.clininfocdmstasksService.GetById(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  loadNewRecord() {
    this.loading = true;
    this.loadRecordSub = this.clininfocdmstasksService.NewCDMSTasksRequest().subscribe(
      (res: any) => {
        if (res.status === 400) {
          this.loading = false;
          return;
  ...
  addOrUpdate() {
    ...
    this.addUpdateSub = this.clininfocdmstasksService.AddorUpdate(this.record).subscribe(
      res => {
        this.form.reset();
        this.location.back();
        this.loading = false;
  ...
  deleteRecord(recId: number) {
    this.loading = true;
    this.deleteRecordSub = this.clininfocdmstasksService.deleteRec(recId).subscribe(
      (res: any) => {
        if (res.status === 400) {
          return;
        } else {
